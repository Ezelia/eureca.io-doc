...
function buildNav(members) {
    var nav = '<h3><a href="index.html">Index</a></h3>',
        seen = {},
        hasClassList = false,
        classNav = '';
		itfNav = '';
    ...
    if (members.classes.length) {
        ...
        members.classes.forEach(function(c) {
            ...
            if ( !hasOwnProp.call(seen, c.longname) ) {
                classNav += '<li>' + linkto(c.longname, '<i class="fa fa-codepen" style="color: #0186d1"></i> ' + c.name);
				
				//we use @see tag to add class methods, this is maybe not the cleanest way
				if (c.see && c.see.length)
				{
					classNav += '<ul>';
					for (var s in c.see)
					{
						classNav += '<li>' + linkto(c.longname+'#'+c.see[s], '<i style="color: #0186d1"></i> ' + c.see[s]+'()');
					}
					classNav += '</ul>';
    ...
    if (members.namespaces.length) {
        ...
        members.namespaces.forEach(function(n) {
            if ( !hasOwnProp.call(seen, n.longname) ) {
                nav += '<li>'+linkto(n.longname, '<i class="fa fa-code"></i> ' + n.name)+'</li>';
            }
    ...
    if (members.events.length) {
        ...
        members.events.forEach(function(e) {
            if ( !hasOwnProp.call(seen, e.longname) && !e.inherited ) {
                if (e.access !== 'private') {
                    nav += '<li>'+linkto(e.longname, '<i class="fa fa-bolt" style="color: yellow"></i> ' + e.name +' <span class="label label-inverse pull-right">' + e.memberof + '</span>')+'</li>';
                    seen[e.longname] = true;
                }
            }
        });
    ...
    if (members.interfaces.length) {
        ...
        members.interfaces.forEach(function(itf) {
            if (!hasOwnProp.call(seen, itf.longname) ) {
                itfNav += '<li>' + linkto(itf.longname, '<i style="color: #0186d1"></i> ' + itf.name)+'</li>';
            }
            seen[itf.longname] = true;
			
        });

        if (itfNav !== '') {
            nav += '<div class="well well-small"><ul class="nav nav-list"><li class="nav-header">Interfaces</li>';
            nav += itfNav;
            nav += '</ul></div>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    for (var longname in helper.longnameToUrl) {
        if ( hasOwnProp.call(helper.longnameToUrl, longname) ) {
            ...
            if (myClasses.length) {
                generate('<i class="fa fa-codepen" style="color:#0186d1"></i> ' + myClasses[0].name, myClasses, helper.longnameToUrl[longname]);
            }
            ...
            if (myNamespaces.length) {
                generate('<i class="fa fa-code"></i> ' + myNamespaces[0].name, myNamespaces, helper.longnameToUrl[longname]);
            }
